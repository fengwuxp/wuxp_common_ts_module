...
function resolveNpmFilesPath(pkgName, isProduction, npmConfig, buildAdapter = BUILD_TYPES.WEAPP, root = basedir, compileInclude = []) {
    ...
    if (!resolvedCache[pkgName]) {
        let res;
        //获取pckName 修正
        if (pkgName.indexOf("src") > 0) {
            // const paths = pkgName.split("/src/");
            // pkgName = paths[0];
            // root = `${root}\\src\\${paths[1]}`;
            // console.log("修正后的pkgName", pkgName, root);
            res = `${root}\\node_modules\\${pkgName}.ts`.replace(/\//g, "\\");
            console.log("res", res)
        } else {
            res = resolveNpmPkgMainPath(pkgName, isProduction, npmConfig, buildAdapter, root)
        }
...
async function recursiveRequire(filePath, files, isProduction, npmConfig = {}, buildAdapter, compileInclude = []) {
    ...
    // console.log("outputNpmPath", outputNpmPath,copyedFiles[outputNpmPath]);
    ...
    if (!copyedFiles[outputNpmPath]) {
        if (compileInclude && compileInclude.length) {
            ...
            // console.log("-npmPkgName->",npmPkgName,nodeModulesIndex,filePathArr);
