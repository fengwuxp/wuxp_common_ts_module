...
export const standardizedWeexModuleToPromise = <T extends WeexStandardizedModule>(options: WeexStandardizedOptions<T>): T => {
    ...
    const proxy = ProxyFactory.newInstance({
        object: module,
        methodInterceptor: (object: any, propertyKey: string, receiver: any) => {
            return (...args) => {
                const func = module[propertyKey];
                return new Promise((resolve, reject) => {
                    const transformParamHandle: TransformParamHandle = getHandleByConfigMap(transformParamMap, propertyKey);
                    const transformCallbackTypeHandle: TransformCallbackTypeHandle = getHandleByConfigMap(transformCallbackMap, propertyKey);
                    func(
                        ...(transformParamHandle ? transformParamHandle(...args) : args),
                        ...(transformCallbackTypeHandle ? transformCallbackTypeHandle(resolve, reject) : [resolve, reject])
                    );
                });
            }
        },
        noSuchMethodInterceptor: (object: any, propertyKey: string, receiver: any) => {
            return (...args) => {
                const enhanceHandle = getHandleByConfigMap(enhanceMap, propertyKey);
                if (enhanceHandle != null) {
                    return enhanceHandle(proxy, ...args);
                } else {
                    throw new Error(`not support method: ${propertyKey}`);
                }
            }
        }
    });
